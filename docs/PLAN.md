...
| Map filter panel opening by itself | Open | ~22 incidents 2026-09-13 to 09-24 (template score >= 0.6 on saved NO_CANDIDATES / FILTER_PANEL_OPEN frames), about 2 a day, one mine each: 3 blank scans, then the city round trip clears it. Latest 2026-09-24 00:52, the first mine after a farm restart (two ALT+TABs at startup). The input before each incident shows no common key (ALT+TAB, SPACE, nothing) -- trigger still unknown |
