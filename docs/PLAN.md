...
| Map types and zones | In progress (operator 2026-09-24: "lam di", KvK in ~1 month) | (1) Per-map size: done 2026-09-24 (see Decisions) -- learned from positions, and measured once on a KvK map by `rok_farm/map_edge.py` (east and north at the widest readable zoom until the HUD names the next kingdom). (2) Zones from the game, `tools/dev/kingdom_survey.py`: the limit is ~15 notches out; from notch 9 the HUD shows no coordinates and the strategic map shows provinces (white lines, names), passes (level 1/2/3, blue = our alliance, red = others) and the filter panel opens by itself ("Lien minh" ticked -- the operator: untick it for a cleaner map). The far view is a tilted perspective (the kingdom's east edge slants) and is held inside the kingdom; notch steps are not deterministic, and `_scroll_at_center` zooms at a random point up to a quarter of the window off centre, which moves the camera -- likely why the drag calibration at the limit disagreed by 15-20%. Next: a projection from landmarks (the kingdom's corners, the city), stitching, provinces and passes per map. (3) No pass-pan case seen yet. Found on the way: kingdoms adjoin at readable zooms (4096 X 1199 -> S11001 X 0; S11001 Y 0 -> 4093 Y 1199) |
