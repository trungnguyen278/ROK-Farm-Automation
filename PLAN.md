...
- [x] Calibrated on the live client 2026-08-14 at 1533x863 (`logs/state_signals.json`): world / city / bag panel. Numbers and their consequences are in `rok_farm/config.py`.
...
| 2026-08-14 | A model-derived click is verified by RESULT, never trusted | Measured twice on one identical frame: gemini answered (768,118) then (690,120). The second was a real X -- the banner's, not the panel's, because that screen has two. Trusting the coordinate would have reported success while the panel stayed open; re-reading the dim ratio catches it |
...
| 2026-08-14 | Liveness and the dim ratio must be read TOGETHER, never separately | Measured: an open bag panel drops frame activity to 0.008, indistinguishable from a crashed client (live floor 0.504). Judging stillness alone would restart the game every time a panel is opened |
